 ...
 * side-effects (orphaned team count) and processed_events row count. Both are 1 even without the
 * fix: (1) {@code merge()} updates the SAME row, so the count stays at 1; (2) re-orphaning an
 * already-orphaned team is naturally idempotent at the business layer. Neither assertion failed
 * when {@code Persistable} was removed.
 *
