 ...
 * {@code @Transactional} is not bypassed via self-invocation. Self-invocation would
 * bypass the proxy, release the {@code FOR UPDATE SKIP LOCKED} row locks before Kafka
 * publish completes, and risk double-publish when the scheduler fires concurrently.
 *
 * <p>Uses {@code FOR UPDATE SKIP LOCKED} queries ({@code lockPendingBatch} /
 * {@code lockRetryBatch}) to prevent duplicate processing when multiple relay threads
 * are active.
 */
@Component
public class OutboxRelayExecutor {
     ...
     *
     * <p>The pending batch ({@code failed_at IS NULL}) and retry batch
