 ...
 * (bad JSON, missing {@code eventId}, unparseable UUID, etc.) and will never succeed on retry.
 * Consumers catch it explicitly to skip the message instead of retrying forever. It is
 * deliberately distinct from a bare {@link NullPointerException} so that an NPE raised by
 * business logic is NOT mistaken for a malformed payload.
 */
